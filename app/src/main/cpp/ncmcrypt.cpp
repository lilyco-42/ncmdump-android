...
{
    // No-op on Android: metadata tag writing is handled by the Java layer
    // or can be done with Android MediaStore APIs after decryption.
    // The decrypted audio file has already been written by Dump().
}
...
std::string NeteaseCrypt::getMetadataJson() const
{
    ...
    std::string json = "{";
    json += "\"name\":\"" + escapeJson(mMetaData->name()) + "\",";
    json += "\"artist\":\"" + escapeJson(mMetaData->artist()) + "\",";
    json += "\"album\":\"" + escapeJson(mMetaData->album()) + "\",";
    json += "\"format\":\"" + escapeJson(mMetaData->format()) + "\",";
    json += "\"duration\":" + std::to_string(mMetaData->duration()) + ",";
    json += "\"bitrate\":" + std::to_string(mMetaData->bitrate());
    json += "}";
    return json;
}

std::string NeteaseCrypt::getCoverArtExtension() const
{
    if (mImageData.empty()) return "";
    // Check PNG magic bytes
    if (mImageData.size() >= 8 && memcmp(mImageData.c_str(), "\x89\x50\x4E\x47\x0D\x0A\x1A\x0A", 8) == 0)
    {
        return "png";
    }
    return "jpg";
